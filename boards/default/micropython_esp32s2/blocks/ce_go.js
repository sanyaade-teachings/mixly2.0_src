'use strict';

goog.provide('Blockly.Blocks.me_go');
goog.require('Blockly.Blocks');

Blockly.Msg['MEGO_HUE'] = 100

//LED
Blockly.Blocks['ce_go_light_number'] = {
    init: function() {
        this.setColour(Blockly.Msg['MEGO_HUE']);
        this.appendDummyInput("")
            .appendField(new Blockly.FieldDropdown([
                [Blockly.Msg.Lang.MIXGO_LEFT_FRONT+Blockly.Msg.Lang.COLOUR_RGB_GREEN+Blockly.Msg.Lang.MIXLY_EXTERN_LED, "GLED[0]"],
                [Blockly.Msg.Lang.MIXGO_RIGHT_FRONT+Blockly.Msg.Lang.COLOUR_RGB_GREEN+Blockly.Msg.Lang.MIXLY_EXTERN_LED, "GLED[1]"],
                [Blockly.Msg.Lang.MIXGO_LEFT_BACK+Blockly.Msg.Lang.COLOUR_RGB_GREEN+Blockly.Msg.Lang.MIXLY_EXTERN_LED, "GLED[3]"],
                [Blockly.Msg.Lang.MIXGO_RIGHT_BACK+Blockly.Msg.Lang.COLOUR_RGB_GREEN+Blockly.Msg.Lang.MIXLY_EXTERN_LED, "GLED[2]"],
                [Blockly.Msg.Lang.MIXGO_LEFT_FRONT+Blockly.Msg.Lang.COLOUR_RGB_RED+Blockly.Msg.Lang.MIXLY_EXTERN_LED, "RLED[0]"],
                [Blockly.Msg.Lang.MIXGO_RIGHT_FRONT+Blockly.Msg.Lang.COLOUR_RGB_RED+Blockly.Msg.Lang.MIXLY_EXTERN_LED, "RLED[1]"],
                [Blockly.Msg.Lang.MIXGO_LEFT_BACK+Blockly.Msg.Lang.COLOUR_RGB_RED+Blockly.Msg.Lang.MIXLY_EXTERN_LED, "RLED[3]"],
                [Blockly.Msg.Lang.MIXGO_RIGHT_BACK+Blockly.Msg.Lang.COLOUR_RGB_RED+Blockly.Msg.Lang.MIXLY_EXTERN_LED, "RLED[2]"],
                [Blockly.Msg.Lang.ME_GO_LIGHT_HEADLIGHT, "WLED"]
            ]), 'op')
        this.setOutput(true);
    }
};

Blockly.Blocks.ce_go_led_bright = {
  init: function() {
    this.setColour(Blockly.Msg['MEGO_HUE']);
    this.appendDummyInput()
    .appendField(Blockly.Msg.Lang.MIXLY_SETTING);
    this.appendValueInput('led')
    .appendField('ME GO')
    this.appendValueInput('bright')
    .appendField(Blockly.Msg.Lang.MIXLY_PULSEIN_STAT)  
    this.setPreviousStatement(true, null);
    this.setNextStatement(true, null);
    this.setInputsInline(true);
    this.setTooltip(Blockly.Msg.Lang.MIXLY_ESP32_LED_SETONOFF);
  }
};

Blockly.Blocks.ce_go_get_led_bright = {
  init: function() {
    this.setColour(Blockly.Msg['MEGO_HUE']);
    this.appendDummyInput()
    .appendField(Blockly.Msg.Lang.MIXLY_MICROBIT_PY_STORAGE_GET);
    this.appendValueInput('led')
    .appendField('ME GO')
    this.appendDummyInput()
    .appendField(Blockly.Msg.Lang.MIXLY_BRIGHTNESS)  
    this.setOutput(true);
    this.setInputsInline(true);
    this.setTooltip(Blockly.Msg.Lang.MIXLY_ESP32_LED_GETONOFF);
  }
};

Blockly.Blocks.ce_go_get_led_state = {
  init: function() {
    this.setColour(Blockly.Msg['MEGO_HUE']);
    this.appendDummyInput()
    .appendField(Blockly.Msg.Lang.MIXLY_MICROBIT_PY_STORAGE_GET);
    this.appendValueInput('led')
    .appendField('ME GO')
    this.appendDummyInput()
    .appendField(Blockly.Msg.Lang.MIXLY_PULSEIN_STAT)  
    this.setOutput(true);
    this.setInputsInline(true);
    this.setTooltip(Blockly.Msg.Lang.MIXLY_ESP32_LED_GETONOFF);
  }
};

Blockly.Blocks.ce_go_led_brightness = {
  init: function() {
    this.setColour(Blockly.Msg['MEGO_HUE']);
    this.appendDummyInput()
    .appendField(Blockly.Msg.Lang.MIXLY_SETTING);
    this.appendValueInput('led')
    .appendField('ME GO')
    this.appendValueInput('bright')
    .appendField(Blockly.Msg.Lang.MIXLY_BRIGHTNESS)
    this.appendDummyInput("")
    .appendField("%")
    this.setPreviousStatement(true, null);
    this.setNextStatement(true, null);
    this.setInputsInline(true);
    this.setTooltip(Blockly.Msg.Lang.MIXLY_SETTING+Blockly.Msg.Lang.MIXLY_BUILDIN_LED+Blockly.Msg.Lang.MIXLY_BRIGHTNESS+'(0-10)');
  }
};


Blockly.Blocks.ce_go_stepper_keep = {
    init: function () {
      this.setColour(Blockly.Msg['MEGO_HUE']);
      this.appendDummyInput()
          .appendField("ME GO")
          .appendField(new Blockly.FieldDropdown([
            [Blockly.Msg.Lang.blockpy_forward, "F"],
            [Blockly.Msg.Lang.blockpy_backward, "B"],
            [Blockly.Msg.Lang.blockpy_left, "L"],
            [Blockly.Msg.Lang.blockpy_right, "R"]            
            ]), "VAR");
      this.appendValueInput('speed')
          .setCheck(Number)
          .setAlign(Blockly.ALIGN_RIGHT)
          .appendField(Blockly.Msg.Lang.MIXLY_STEPPER_SET_SPEED);
      this.appendDummyInput("")
          .appendField("%")    
      this.setPreviousStatement(true, null);
      this.setNextStatement(true, null);
      this.setInputsInline(true);
    }
}

Blockly.Blocks.ce_go_stepper_stop = {
    init: function () {
      this.setColour(Blockly.Msg['MEGO_HUE']);
      this.appendDummyInput()
          .appendField("ME GO")
          .appendField(new Blockly.FieldDropdown([           
            [Blockly.Msg.Lang.MOTOR_P, "P"],
            [Blockly.Msg.Lang.MOTOR_N, "N"]
            ]), "VAR");      
      this.setPreviousStatement(true, null);
      this.setNextStatement(true, null);
      this.setInputsInline(true);
    }
}

Blockly.Blocks.ce_go_dc_motor = {
    init: function () {
      this.setColour(Blockly.Msg['MEGO_HUE']);
      this.appendDummyInput()
          .appendField("ME GO")
          .appendField(Blockly.Msg.Lang.MOTOR_DC)
          .appendField(new Blockly.FieldDropdown([
            [Blockly.Msg.Lang.MIXLYCAR_WHEEL_LEFT, "0"],
            [Blockly.Msg.Lang.MIXLYCAR_WHEEL_RIGHT, "1"],
            [Blockly.Msg.Lang.ME_GO_MOTOR_EXTERN, "2"]
            ]), "wheel");
      this.appendDummyInput()
          .appendField(Blockly.Msg.Lang.MIXLY_MICROBIT_Direction)
                  .appendField(new Blockly.FieldDropdown([
            [Blockly.Msg.Lang.ROTATION_FORWARD, "CW"],
            [Blockly.Msg.Lang.ROTATION_BACKWARD, "CCW"],
            [Blockly.Msg.Lang.MOTOR_P, "P"],
            [Blockly.Msg.Lang.MOTOR_N, "N"]
            ]), "direction");
      this.appendValueInput('speed')
          .setCheck(Number)
          .setAlign(Blockly.ALIGN_RIGHT)
          .appendField(Blockly.Msg.Lang.MIXLY_STEPPER_SET_SPEED);
      this.appendDummyInput("")
          .appendField("%")    
      this.setPreviousStatement(true, null);
      this.setNextStatement(true, null);
      this.setInputsInline(true);
    }
}

Blockly.Blocks.ce_go_hall_attachInterrupt = {
  init: function() {
    this.setColour(Blockly.Msg['MEGO_HUE']);
    this.appendDummyInput("")
    .appendField(Blockly.Msg.Lang.MIXLY_ESP32_INTERRUPT)
    .appendField(Blockly.Msg.Lang.ME_GO_HALL_SENSOR)
    this.appendDummyInput("")
    .appendField(new Blockly.FieldDropdown([[Blockly.Msg.Lang.TEXT_TRIM_LEFT, "A"], [Blockly.Msg.Lang.TEXT_TRIM_RIGHT, "B"]]), "mode");
    this.appendValueInput('DO')
        .appendField(Blockly.Msg.Lang.MIXLY_DO)
    this.setPreviousStatement(true);
    this.setNextStatement(true);
    this.setInputsInline(true);
}
};

Blockly.Blocks.ce_go_hall_initialize = {
  init: function() {
    this.setColour(Blockly.Msg['MEGO_HUE']);
    this.appendDummyInput("")    
    .appendField(Blockly.Msg.Lang.ME_GO_HALL_SENSOR)    
    this.appendDummyInput("")
    .appendField(new Blockly.FieldDropdown([[Blockly.Msg.Lang.TEXT_TRIM_LEFT, "A"], [Blockly.Msg.Lang.TEXT_TRIM_RIGHT, "B"]]), "mode");
    this.appendDummyInput("")
        .appendField(Blockly.Msg.Lang.MIXLY_SETUP)
    this.appendDummyInput("")
      .appendField(new Blockly.FieldDropdown([['turns', "turns"], ['distance', "distance"],['turns,distance','all']]), "args");    
    this.appendValueInput('num')
          .setCheck(Number)    
    this.setPreviousStatement(true);
    this.setNextStatement(true);
    this.setInputsInline(true);
}
};

Blockly.Blocks.ce_go_hall_data = {
  init: function() {
    this.setColour(Blockly.Msg['MEGO_HUE']);
    this.appendDummyInput("")    
    .appendField(Blockly.Msg.Lang.ME_GO_HALL_SENSOR)    
    this.appendDummyInput("")
    .appendField(new Blockly.FieldDropdown([[Blockly.Msg.Lang.TEXT_TRIM_LEFT, "A"], [Blockly.Msg.Lang.TEXT_TRIM_RIGHT, "B"]]), "mode");
    this.appendDummyInput("")
        .appendField(Blockly.Msg.Lang.MIXLY_MICROBIT_PY_STORAGE_GET)
    this.appendDummyInput("")
      .appendField(new Blockly.FieldDropdown([['turns', "turns"], ['distance', "distance"],['speed','speed']]), "args");    
    this.setOutput(true);
    this.setInputsInline(true);
}
};

Blockly.Blocks['ce_go_pin_near_line'] = {
    init: function(){
        this.setColour(Blockly.Msg['MEGO_HUE']);
        this.appendDummyInput()
            .appendField(Blockly.Msg.Lang.MIXLY_MICROBIT_PY_STORAGE_GET)
            .appendField(new Blockly.FieldDropdown([
                [Blockly.Msg.Lang.TEXT_TRIM_LEFT, "[0]"], 
                [Blockly.Msg.Lang.MIXGO_LEFT_MID, "[1]"],
                [Blockly.Msg.Lang.MIXGO_RIGHT_MID, "[2]"],
                [Blockly.Msg.Lang.TEXT_TRIM_RIGHT, "[3]"],
                [Blockly.Msg.Lang.MIXLY_ALL, ""]
                ]), "key")
            .appendField(Blockly.Msg.Lang.MIXGO_LINE_SENSOR_VAL);
        this.setOutput(true,Number);
        this.setInputsInline(true);
        var thisBlock = this;
        this.setTooltip(function() {
            var mode0 = Blockly.Msg.Lang.MIXLY_ESP32_SENSOR_MIXGO_PIN_NEAR_TOOLTIP;
            var mode1 = Blockly.Msg.Lang.MIXLY_ESP32_NEAR;
            return mode0 + mode1
        });
    }
};

Blockly.Blocks['ce_go_pin_near'] = {
    init: function(){
        this.setColour(Blockly.Msg['MEGO_HUE']);
        this.appendDummyInput()
            .appendField(Blockly.Msg.Lang.MIXLY_MICROBIT_PY_STORAGE_GET)
            .appendField(new Blockly.FieldDropdown([
                [Blockly.Msg.Lang.MIXGO_LEFT_FRONT, "[0]"], 
                [Blockly.Msg.Lang.MIXGO_RIGHT_FRONT, "[1]"],
                [Blockly.Msg.Lang.MIXGO_LEFT_BACK, "[3]"],
                [Blockly.Msg.Lang.MIXGO_RIGHT_BACK, "[2]"],
                [Blockly.Msg.Lang.MIXLY_ALL, ""]
                ]), "key")
            .appendField(Blockly.Msg.Lang.MIXGO_PROXIMITY_SENSOR);
        this.setOutput(true,Number);
        this.setInputsInline(true);
        var thisBlock = this;
        this.setTooltip(function() {
            var mode0 = Blockly.Msg.Lang.MIXLY_ESP32_SENSOR_MIXGO_PIN_NEAR_TOOLTIP;
            var mode1 = Blockly.Msg.Lang.MIXLY_ESP32_NEAR;
            return mode0 + mode1
        });
    }
};

Blockly.Blocks['ce_go_pin_near_state_change'] = {
    init: function(){
        this.setColour(Blockly.Msg['MEGO_HUE']);
        this.appendDummyInput()
            .appendField(Blockly.Msg.Lang.MIXLY_MIXGO_CAR_SENSOR_ONBOARD_CHANGE)
            .appendField(new Blockly.FieldDropdown([
                [Blockly.Msg.Lang.MIXLY_ME_GO_CAR_SENSOR_ONBOARD_AUTO_CHANGE, "AS"],
                [Blockly.Msg.Lang.MIXLY_MIXGO_CAR_USE_LINE_ONLY, "LP"], 
                [Blockly.Msg.Lang.MIXLY_MIXGO_CAR_USE_PROXIMITY_ONLY, "OA"],
                [Blockly.Msg.Lang.MIXLY_ME_GO_CAR_LIGHT_SEEKING_ONLY, "LS"],
                [Blockly.Msg.Lang.ME_GO_SENSOR_MODE_OFF, "CL"]
                ]), "key");
        this.setPreviousStatement(true, null);
        this.setNextStatement(true, null);
        this.setInputsInline(true);
    }
};

Blockly.Blocks['sensor_mixgome_eulerangles'] = {
    init: function(){
        this.setColour(Blockly.Msg['MEGO_HUE']);
        this.appendDummyInput("")
            .appendField(Blockly.Msg.Lang.MIXLY_GET_GESTURE)   
            .appendField(new Blockly.FieldDropdown([
                [Blockly.Msg.Lang.ME_GO_PITCH,'[0]'],
                [Blockly.Msg.Lang.ME_GO_ROLL,'[1]'],
                [Blockly.Msg.Lang.ME_GO_PITCH+', '+Blockly.Msg.Lang.ME_GO_ROLL,'']
                ]),'angle');         
        // this.appendDummyInput("")
        //     .appendField(Blockly.Msg.Lang.BOARD_DIRECTION)   
        //     .appendField(new Blockly.FieldDropdown([
        //         [Blockly.Msg.Lang.OLED_VER,'True'],
        //         [Blockly.Msg.Lang.OLED_HOR,'False'],                
        //         ]),'dir');             
        this.appendDummyInput("")        
        this.setOutput(true, Number);
        this.setInputsInline(true);
        
    }
};

Blockly.Blocks['ce_go_pin_light'] = {
    init: function(){
        this.setColour(Blockly.Msg['MEGO_HUE']);
        this.appendDummyInput()
            .appendField(Blockly.Msg.Lang.MIXLY_MICROBIT_PY_STORAGE_GET)
            .appendField(new Blockly.FieldDropdown([
                [Blockly.Msg.Lang.TEXT_TRIM_LEFT, "[0]"], 
                [Blockly.Msg.Lang.MIXGO_LEFT_MID, "[1]"],
                [Blockly.Msg.Lang.MIXGO_RIGHT_MID, "[2]"],
                [Blockly.Msg.Lang.TEXT_TRIM_RIGHT, "[3]"],
                [Blockly.Msg.Lang.MIXLY_ALL, ""]
                ]), "key")
            .appendField(Blockly.Msg.Lang.MIXLY_ME_GO_CAR_LIGHT_SEEKING_SENSOR);
        this.setOutput(true,Number);
        this.setInputsInline(true);
        var thisBlock = this;
        this.setTooltip(function() {
            var mode0 = Blockly.Msg.Lang.MIXLY_ESP32_SENSOR_MIXGO_PIN_NEAR_TOOLTIP;
            var mode1 = Blockly.Msg.Lang.MIXLY_ME_GO_CAR_LIGHT_SEEKING_SENSOR;
            return mode0 + mode1
        });
    }
};